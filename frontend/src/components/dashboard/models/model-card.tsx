...
import { Play, X, Loader2, Upload, Cpu, ChevronRight, CheckCircle2, Info } from "lucide-react";
...
const BASE_MODEL_INFO: Record<string, { params: string; useCase: string; speed: string }> = {
  tinyllama: {
    params: "1.1B",
    speed: "Fast · CPU-friendly",
    useCase: "Great for Q&A, classification, and short-form generation on low-resource hardware.",
  },
  qwen: {
    params: "0.5B",
    speed: "Very fast · minimal VRAM",
    useCase: "Ultra-compact model. Good for multilingual tasks and rapid prototyping.",
  },
};

function InlineProgress({ logs, isDone, isTraining }: { logs: string[]; isDone: boolean; isTraining: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between">
        ...
        {pct !== null && <span className="text-[11px] font-medium">{pct}%</span>}
      </div>
...
export function ModelCard({ model }: { model: ModelSummary }) {
  ...
  const { logs, isDone } = useTrainingLogs(isTraining ? model.id : null, isTraining);

  const handleStartTraining = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!dataset) { toast.error("Dataset not found for this model."); return; }
  ...
  const handleDelete = (e: React.MouseEvent) => {
    ...
    deleteModel.mutate(model.id, {
      onSuccess: () => { toast.success(`"${model.display_name}" removed.`); setConfirmDelete(false); },
      onError: (err) => { toast.error(err.message); setConfirmDelete(false); },
    });
  };
        ...
        {model.is_base_model && baseInfo && (
          <div className="rounded-md border bg-muted/30 px-3 py-2">
            <div className="flex items-center justify-between gap-2">
              <span className="text-[11px] font-medium">{baseInfo.params} params · {baseInfo.speed}</span>
              <button
                onClick={(e) => { e.stopPropagation(); setShowBaseInfo((v) => !v); }}
            ...
            {showBaseInfo && (
              <p className="mt-1.5 text-[11px] leading-4 text-muted-foreground">{baseInfo.useCase}</p>
            )}
          </div>
        )}
